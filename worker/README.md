...
- `IntervalsMcp` (`src/index.ts`) : un [`McpAgent`](https://github.com/cloudflare/agents) — un Durable Object par session cliente, qui enregistre les mêmes 9 tools que la version stdio.
...
4. Entre ton `OWNER_PASSWORD`. Une fois validé, claude.ai reçoit un jeton d'accès (durée par défaut : 1h, renouvelable via refresh token ~30 jours) et les 9 tools apparaissent (dont 3 en écriture).
